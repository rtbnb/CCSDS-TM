----------------------------------------------------------------
-- File : width_conversion_fifo_tb_3.vhd
-- Created : 23.04.2026
-- Author : Lukas Reil 
...
entity width_conversion_fifo_tb_3 is
end entity width_conversion_fifo_tb_3;

architecture behavioral of width_conversion_fifo_tb_3 is

    constant INPUT_DATA_WIDTH : integer := 8;
    constant OUTPUT_DATA_WIDTH : integer := 4;
    constant DEPTH      : integer := 16;

    signal wr_clk_r   : std_logic := '0';
    signal wr_en_r    : std_logic := '0';
    signal wr_data_r  : std_logic_vector(INPUT_DATA_WIDTH-1 downto 0) := (others => '0');
    signal full_s     : std_logic;

    signal rd_clk_r   : std_logic := '0';
    signal rd_en_r    : std_logic := '0';
    signal rd_data_s  : std_logic_vector(OUTPUT_DATA_WIDTH-1 downto 0);
    ...
    component width_conversion_fifo is
        generic (
            input_data_width_g : integer := INPUT_DATA_WIDTH;
            output_data_width_g : integer := OUTPUT_DATA_WIDTH;
...
begin
    ...
    fifo : width_conversion_fifo
        generic map (
            input_data_width_g => INPUT_DATA_WIDTH,
            output_data_width_g => OUTPUT_DATA_WIDTH,
            depth_g      => DEPTH
        )
        port map (
            wr_clk_i   => wr_clk_r,
    ...
    begin
        ...
        for i in 0 to 15 loop
            wr_en_r <= '1';
            wr_data_r <= std_logic_vector(to_unsigned(i, INPUT_DATA_WIDTH));
            wait for WR_CLK_PERIOD;
        end loop;
        wr_en_r <= '0';
        wait;
