...
architecture behavioral of virtual_channel_buffer is
    ...
    signal space_packet_read_ptr_r: integer range 0 to SPACE_PACKET_MAX_SIZE -1 := 0;
...
begin
    ...
    space_packet_size_s <= to_integer(signed(space_packet_data_length_s)) + SPACE_PACKET_PRIMARY_HEADER_SIZE - 1;
    ...
    begin
        if rising_edge(clk_i) and reset_i = '1' then
            if space_packet_decoding_state_r = RESET then
                ...
                space_packet_decoding_state_r <= WRITE;
                ...
                if space_packet_write_ptr_r > SPACE_PACKET_PRIMARY_HEADER_SIZE and space_packet_write_ptr_r = space_packet_size_s then
                    space_packet_write_ptr_r <= 0;
